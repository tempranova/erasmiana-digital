'use client';

import React, { useEffect, useState, useRef } from 'react';

export default function SingleText({ book }) {

  const containerRef = useRef();
  const [ view, setView ] = useState('sections')
  const [ currentTranslation, setCurrentTranslation ] = useState(0)
  const [ divider, setDivider ] = useState(50);

  // useEffect(() => {
  //   let params = new URLSearchParams(window.location.search)
  //   if(params.get("view")) {
  //     setView(params.get("view"))
  //   }
  // }, [])

  // useEffect(() => {
  //   const url = new URL(window.location)
  //   url.searchParams.set("view", view)
  //   window.history.replaceState({}, "", url);
  // }, [view])

  const scrollTranslation = (section, sectionIndex) => {
    const totalChars = book.sections.reduce((sum, s) => sum + s.text.length, 0);
    const charsBefore = book.sections
      .slice(0, sectionIndex)
      .reduce((sum, s) => sum + s.text.length, 0);

    const translationContainer = document.getElementById('translation-container');
    translationContainer.scrollTo({
      top: (charsBefore / totalChars) * (translationContainer.scrollHeight - translationContainer.clientHeight),
      behavior: "smooth",
    });
  }
  
  const startDrag = (e) => {
    e.preventDefault();
    const container = containerRef.current;
    const startX = e.clientX;
    const startWidth = divider;

    const onMove = (moveEvent) => {
      const rect = container.getBoundingClientRect();
      const newPercent =
        ((moveEvent.clientX - rect.left) / rect.width) * 100;
      setDivider(Math.min(90, Math.max(10, newPercent))); // clamp 10–90%
    };

    const onUp = () => {
      window.removeEventListener("mousemove", onMove);
      window.removeEventListener("mouseup", onUp);
    };

    window.addEventListener("mousemove", onMove);
    window.addEventListener("mouseup", onUp);
  };

  return (
    <div>
      <div ref={containerRef} className="flex cardo-regular text-left ">
        <div style={{ width: `${divider}%` }} className="border-1 border-gray-200">
          <div className="p-8 border-b-1 border-gray-200 cardo-regular-italic">
            <div className="flex items-center gap-8">
              <div className={`${view === 'full' ? "underline underline-offset-2" : ""} text-black cursor-pointer hover:underline`} onClick={() => setView('full')}>
                Latin (full text)
              </div>
              <div className={`${view === 'sections' ? "underline underline-offset-2" : ""} text-black cursor-pointer hover:underline`} onClick={() => setView('sections')}>
                Latin (sections)
              </div>
            </div>
          </div>
          <div className="p-8 whitespace-pre-line leading-4 max-h-screen overflow-y-auto">
            {book.sections.map((section, i) => {
              return (
                <div key={`section-${i}`} className="">
                  {view === 'sections' ? 
                    <details key={`letter-group-${i}`} className="mt-4 border border-gray-300 rounded-md p-4">
                      <summary className="text-xl cursor-pointer hover:text-gray-500" onClick={() => scrollTranslation(section, i)}>
                        <div className="inline font-semibold">{section.title}</div>
                        <span className="pl-4 mt-2 cardo-regular text-sm text-gray-500">{section.summary.text}</span>
                      </summary>
                      <div className="mt-4 whitespace-pre-line">
                        <p className="mb-4 text-justify cardo-regular leading-8">{section.text}</p>
                      </div>
                    </details>
                  : false}
                  {view === 'full' ? 
                    <div className="mt-4 whitespace-pre-line">
                      <p className="mb-4 text-justify cardo-regular leading-8">{section.text}</p>
                    </div>
                  : false}
                </div>
              )
            })}
          </div>
        </div>

        <div
          onMouseDown={startDrag}
          className="w-2 bg-gray-300 hover:bg-gray-400 cursor-col-resize"
        />

        <div style={{ width: `${100 - divider}%` }} className="flex-1 border-1 border-l-0 border-gray-200">
          <div className="p-8 border-b-1 border-gray-200 cardo-regular-italic capitalize">
            <div className="flex items-center gap-8">
              {book.translations.map((translation, i) => {
                return (
                  <div key={`translation-${i}`} className={`${i === currentTranslation ? "underline underline-offset-2" : ""} text-black cursor-pointer hover:underline`} onClick={() => setCurrentTranslation(i)}>
                    {translation.language} ({translation.translator})
                  </div>
                )
              })}
            </div>
          </div>
          <div id="translation-container" className="p-8 pt-6 whitespace-pre-line leading-8 max-h-screen overflow-y-auto">
            {book.translations[currentTranslation] ? 
              <div>
                {book.translations[currentTranslation].text ? book.translations[currentTranslation].text : false}
                {book.translations[currentTranslation].url ? 
                  <div>
                    <a className="text-green-800 italic flex items-center" href={book.translations[currentTranslation].url} target="_blank">
                      {book.translations[currentTranslation].title}
                      <svg className="ml-2" width="15" height="15" viewBox="0 0 20 20" fill="#2E7D32" stroke="#2E7D32" xmlns="http://www.w3.org/2000/svg">
                        <path d="M9.16699 3.33301C9.62708 3.33318 10 3.70686 10 4.16699C9.99982 4.62697 9.62697 4.99982 9.16699 5H4.16699C3.94598 5 3.73343 5.08786 3.57715 5.24414C3.42105 5.40033 3.33309 5.61219 3.33301 5.83301V15.833C3.33301 16.054 3.42087 16.2666 3.57715 16.4229C3.73343 16.5791 3.94598 16.667 4.16699 16.667H14.167C14.3878 16.6669 14.5997 16.579 14.7559 16.4229C14.9121 16.2666 15 16.054 15 15.833V10.833C15.0002 10.373 15.373 10.0002 15.833 10C16.2931 10 16.6668 10.3729 16.667 10.833V15.833C16.667 16.4959 16.4032 17.1318 15.9346 17.6006C15.4658 18.0693 14.8299 18.3329 14.167 18.333H4.16699C3.50395 18.333 2.86825 18.0694 2.39941 17.6006C1.93057 17.1317 1.66699 16.496 1.66699 15.833V5.83301C1.66708 5.17008 1.93065 4.53419 2.39941 4.06543C2.86822 3.59679 3.5041 3.33301 4.16699 3.33301H9.16699ZM17.5 1.66699C17.5419 1.66699 17.5828 1.6708 17.623 1.67676C17.637 1.67883 17.6512 1.67983 17.665 1.68262C17.6796 1.68553 17.6938 1.6897 17.708 1.69336C17.7184 1.69603 17.729 1.6981 17.7393 1.70117C17.7525 1.70514 17.7654 1.71025 17.7783 1.71484C17.8918 1.75498 17.9981 1.82035 18.0889 1.91113C18.1188 1.94103 18.1455 1.97286 18.1699 2.00586C18.1782 2.01712 18.1866 2.02835 18.1943 2.04004C18.2322 2.09711 18.2618 2.15785 18.2842 2.2207C18.2888 2.23367 18.2939 2.24651 18.2979 2.25977C18.3093 2.29789 18.3164 2.33686 18.3223 2.37598C18.3283 2.41653 18.333 2.45776 18.333 2.5V7.5C18.333 7.96024 17.9602 8.33301 17.5 8.33301C17.0398 8.33301 16.667 7.96024 16.667 7.5V4.51074L10.5889 10.5889C10.2634 10.9143 9.73656 10.9143 9.41113 10.5889C9.08572 10.2634 9.08573 9.73657 9.41113 9.41113L15.4893 3.33301H12.5C12.0398 3.33301 11.667 2.96024 11.667 2.5C11.667 2.03976 12.0398 1.66699 12.5 1.66699H17.5Z" fill="#23282B"/>
                      </svg>
                    </a>
                  </div>
                : false}
              </div>
            : false}
          </div>
        </div>
      </div>
    </div>
  )

}