...
export default function SingleText({ letter }) {
        ...
        <div className="border-1 border border-gray-200">
          <div className="p-8 border-b-1 border-gray-200 cardo-regular-italic capitalize">
            <div className="flex items-center gap-8">
              ...
              <div className={`${letter.commentary.length === currentCommentary ? "underline underline-offset-2" : ""} text-black cursor-pointer hover:underline`} onClick={() => setCurrentCommentary(letter.commentary.length)}>
                AI Summary
              </div>
            </div>
          </div>
          <div className="p-8 pt-6 whitespace-pre-line leading-8">
            {letter.commentary[currentCommentary] ? 
              <div>
                {letter.commentary[currentCommentary].text ? letter.commentary[currentCommentary].text : false}
                {letter.commentary[currentCommentary].url ? 
                  <div>
                    <a className="text-green-800 italic flex items-center" href={letter.commentary[currentCommentary].url} target="_blank">
                      {letter.commentary[currentCommentary].title}
            ...
            : false}
            {currentCommentary === letter.commentary.length ? 
              letter.summary.text
            : false}
          </div>
        </div>
