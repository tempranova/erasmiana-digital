      ...
      {results.map((result, i) => {
        return (
          <div key={`result-${i}`} className="bg-white rounded-md p-4 relative pb-18">
            <div className="capitalize text-2xl im-fell-dw-pica-regular">{result.work.title.toLowerCase()}</div>
            <div className="capitalize text-xl im-fell-dw-pica-regular text-gray-600">Letter {result.metadata.reference}. {result.work.alt_title.toLowerCase()}</div>
            <div className="text-right im-fell-dw-pica-regular-italic text-gray-600">{result.metadata.placename}, {result.metadata.date_text}</div>
            <div className="mt-4 text-sm cardo-regular">{result.summary}</div>
            <Link href={`/letters/${result.work.id}`}>
