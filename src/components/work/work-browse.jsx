      ...
      <div>
        ...
        <div>
          {allSections.map((section, i) => {
            return (
              <div key={`section-${i}`} className="pb-4 mb-4 border-b-1 border-[#3b2d2b]">
                <div>
                  <Link href={`/works/${work.id}/sections/${section.id}`} className="hover:opacity-80 cursor-pointer inline-flex items-center">
                    <div className="text-lg italic">Page{section.pages.length > 1 ? 's' : ''} {section.pages.join(', ')}</div>
                    <svg className="ml-2 -mt-1" width="15" height="15" viewBox="0 0 20 20" strokeWidth="0.5" stroke="#3b2d2b" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 justify-start content-start text-xs">
                    {section.keywords.map((keyword, i) => {
                      return (
                        <div key={`keyword-${i}`}>
                          <div className="border border-[#3b2d2b] px-2 py-0.5 rounded-md opacity-50">
                            {keyword}
                          </div>
                        </div>
                      )
