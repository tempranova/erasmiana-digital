    ...
    <div className="flex items-center justify-center">
      {audioOn ? 
        <svg className="cursor-pointer" onClick={() => {
          ...
          setAudioOn(false)
        }} fill="#3b2d2b" width="20px" height="20px" viewBox="0 0 1920 1920" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="ml-4 flex items-end gap-[2px] h-6">
        {bars.map((h, i) => (
          <div
            key={i}
            className={`${audioOn ? 'bg-[#3b2d2b]' : 'bg-[#3b2d2b]/50'} rounded-sm w-[3px] transition-all duration-150 ease-in-out`}
            style={{ height: `${h}px` }}
          />
        ))}
      </div>
