...
export default async function Page() {
  ...
  const allBooks = await db.selectFrom('Book')
      .select((eb) => [
        'Book.id', 'Book.title', 'Book.alt_title'
      ])
      .execute()
    ...
    <div className="w-full min-h-screen h-full bg-[url('/assets/erasmus-bg-2.png')] bg-cover bg-center bg-no-repeat bg-[#1d1f1b] bg-blend-overlay p-16">
      <div className="max-w-7xl bg-white/90 relative p-8">
        <div className="text-xl cardo-regular">
          <h1 className="im-fell-dw-pica-regular text-4xl mb-4">The Books of Erasmus</h1>
          <p>Erasmus wrote and published over 1000 books during his lifetime.</p>
        </div>
            ...
            <div className="mt-4">
              <div className="bg-white w-full rounded-md p-4">
                ...
                <div className="grid grid-cols-2 gap-8">
                  {allBooks.map((book, i) => {
                    return (
                      <div key={`book-${i}`} className="bg-white rounded-md p-4 relative pb-18">
                        <div className="capitalize text-2xl im-fell-dw-pica-regular">{book.title.toLowerCase()}</div>
                        <div className="capitalize text-xl im-fell-dw-pica-regular text-gray-600">{book.alt_title.toLowerCase()}</div>
